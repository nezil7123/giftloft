            ...
            <div v-if="importing" class="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center" @click.self="closeImport">
                <div class="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-7">
                    ...
                    <form @submit.prevent="runImport" class="mt-5 space-y-4">
                        ...
                        <p class="text-xs text-neutral-400">Product images are downloaded and stored on ComeYay so they keep working even after the Shopify store closes. Imported products land as new catalog entries — review categories, add an emoji/accent, and toggle visibility from the table afterwards. Products already in the catalog (matched by Shopify handle) are skipped, so it's safe to re-run.</p>
