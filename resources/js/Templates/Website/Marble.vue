...
<template>
    <div ref="root" class="text-[#33302a]" :style="`background:${marble}`">
        ...
        <section class="px-6 py-28 sm:px-10">
            <div v-if="showRegistry" data-reveal="zoom" class="mx-auto max-w-5xl overflow-hidden rounded-[2.5rem] border border-amber-600/25 bg-white/60 px-8 py-20 text-center backdrop-blur-sm">
            ...
            <p class="mt-10 text-center text-xs uppercase tracking-[0.3em] text-amber-700/60">{{ event.user?.name }} · ComeYay</p>
        </section>
    </div>
</template>
